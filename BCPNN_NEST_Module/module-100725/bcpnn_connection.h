 ...
 *  bcpnn_connection.h
 *
 *  Written by Philip Tully, Bernhard Kaplan and Nikolaos Chrysanthidis
 *
 */
...
/* BeginDocumentation
  ...
  SecondVersion: March 2012
  Authors: Philip Tully, Bernhard Kaplan
           tully@csc.kth.se, bkaplan@kth.se

  CurrentVersion: February 2020
  Author: Nikolaos Chrysanthidis 
          nchr@kth.se
...
namespace mynest
{
  class BCPNNConnection : public nest::ConnectionHetWD
  {
    ...
    private:
      ...
      std::queue<nest::double_t> history;      
  ...
  void BCPNNConnection::check_connection(nest::Node & s, nest::Node & r, nest::port receptor_type, nest::double_t t_lastspike)
  {
    ...
    r.register_stdp_connection(t_lastspike);
  }
  ...
  void BCPNNConnection::send(nest::Event& e, nest::double_t t_lastspike, const nest::CommonSynapseProperties &)
  {
    ...
    history.push(t_spike + dendritic_delay);
    ...
    for (nest::int_t timestep = 0; timestep < number_iterations; timestep++){
        ...
        if(abs((t_lastspike + timestep*resolution) - history.front()) < 1.0e-8 && t_lastspike != 0.) {
